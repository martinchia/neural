...
z3 = a2_new * Theta2';
a3 = sigmoid(z3);
y_y = zeros(size(y,1),num_labels);
for i = 1:size(y,1)
    y_y(i,y(i)) = 1;
end
% [n,id] = max(a3');
% y_pred = zeros(size(y,1),10);
% for i = 1:size(id,2)
%     y_pred(i,id(i)) = 1;
% end
delta3 = a3 - y_y;
...
Delta2 = delta3' * a2_new;
Delta1 = delta2(:,2:end)' * X_new;
...
Jm = sum(y_y.*log(a3)+ ...
    (ones(size(y_y))-y_y).*log(ones(size(y_y)) - a3));

grad = [Theta1_grad(:);Theta2_grad(:)];
t1 = Theta1(:,2:end);
t2 = Theta2(:,2:end);
J = (-1/m)*sum(Jm) + (lambda/(2*m)).*sum([t1(:);t2(:)].^2);
% ====================== YOUR CODE HERE ======================
